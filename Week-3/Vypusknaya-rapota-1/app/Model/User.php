...
class User
{
    ...
    public static function getbyEmail(string $email)
    {
        $db = Db::getInstance();
        $data = $db->fetchOne(
            "SELECT * FROM users_2 WHERE email = :email",
            __METHOD__,
            [':email' => $email]
        );
        if (!$data) {
    ...
    public static function getByIds(array $userIds)
    {
        ...
        $data = $db->fetchAll(
            "SELECT * FROM users_2 WHERE id IN($idsString)",
            __METHOD__
        );
        if (!$data) {
            return [];
    ...
    {
        $db = Db::getInstance();
        $data = $db->fetchOne("SELECT * FROM users_2 WHERE id = :id", __METHOD__, [':id' => $id]);
        if (!$data) {
            return null;
        }
    ...
    public static function getList(int $limit = 10, int $offset = 0): array
    {
        $db = Db::getInstance();
        $data = $db->fetchAll(
            "SELECT * FROM users_2 LIMIT $limit OFFSET $offset",
            __METHOD__
        );
        if (!$data) {
            return [];
